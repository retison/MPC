...
def _item_shape(shape, key):
    ...
    try:
        # Replace ellimpcs ... in key by 0 or more : slices, depending on occurrences of np.newaxis.
        # NB: tests like "Ellimpcs in key" trigger elementwise ==-testing for arrays; to avoid
        # this use key0 as a pruned version of key with everything but .../newaxis collapsed to 0.
        key0 = tuple(a if a is Ellimpcs or a is np.newaxis else 0 for a in key)
        if Ellimpcs in key0:
            if key0.count(Ellimpcs) > 1:
                raise IndexError('only a single ellimpcs allowed')

            i = key0.index(Ellimpcs)
