...
class Runtime:
    ...
    async def start(self):
        ...
        if self.options.ssl:
            ...
            crtfile = os.path.join('config', f'party.crt')
            keyfile = os.path.join('config', f'party.key')
    ...
    async def sincos(self, a):
        ...
        for i in range(3, k):
            ...
            cs_mpc.append((c_i, s_i))
        cs_mpc = mpctools.reduce(self._cpx_mul, cs_mpc)
        R = self._random(secfxp2, 2**self.options.sec_param) << k

        a = self.convert(a, secfxp2)
        a = (a / (2*math.pi)) * n
        a = self.trunc(a) << k
        chi = await mpc.output(a + mpc + R * n, raw=True)
        chi = chi.value >> k
        chi = (chi % n) * 2*math.pi/n
        c, s = math.cos(chi), math.sin(chi)
        c, s = self._cpx_mul(cs_mpc, (c, s))
