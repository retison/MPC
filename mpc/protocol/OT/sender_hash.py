...
# 这几个哈希函数是否过于不安全了呢？？？
class sender_hash:
    def __init__(self, n, s, key_lenth, salt):
        self.n = n
        self.s = s
        self.key_lenth = key_lenth
        self.salt = salt
        # 操作表，似乎为了安全多加会更好？
    ...
    def hash1(self, key):
        data = (1 + key + self.salt).to_bytes(self.key_lenth, "big")
    ...
    def hash2(self, key):
        data = (2 + key + self.salt).to_bytes(self.key_lenth, "big")
        length = (self.n.bit_length() + 7) // 8
        h = hashlib.sha256(data).digest()[:length]
        val = int.from_bytes(h, "big")
        return val % self.n
    ...
    def hash3(self, key):
        data = (3 + key + self.salt).to_bytes(self.key_lenth, "big")
        length = (self.n.bit_length() + 7) // 8
        h = hashlib.sha256(data).digest()[:length]
        val = int.from_bytes(h, "big")
        return val % self.n
    ...
    def hash4(self, key):
        data = (4 + key + self.salt).to_bytes(self.key_lenth, "big")
        length = (self.n.bit_length() + 7) // 8
        h = hashlib.sha256(data).digest()[:length]
        val = int.from_bytes(h, "big")
        return val % self.n
    ...
    def hash5(self, key):
        data = (5 + key + self.salt).to_bytes(self.key_lenth, "big")
        length = (self.n.bit_length() + 7) // 8
        h = hashlib.sha256(data).digest()[:length]
        val = int.from_bytes(h, "big")
        return val % self.n
