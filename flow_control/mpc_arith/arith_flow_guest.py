# test cmd 
# ipython -i flow_control/mpc_arith/aggre_flow_guest.py
import argparse
import asyncio
import json
...
from flow_control.mpc_arith.utils import list_operation
...
# 这里需要继承 base 
class ArithFlowGuest(FlowControllerBase):
    # 这里就是作为发起方，在 任务 ready 之后
    # 作为回调函数，利用接口完成剩余的内容
    def __init__(self, job_id):
        self.job_id = job_id
        super(ArithFlowGuest, self).__init__(job_id)
    ...
    def run_protocol(self):
        ...
        if len(self.party_list) > 0:
            ...
            self.logger.info(f"Send config finish")
            self.logger.info(f"Start get free port")
            info_list = []
            ...
            if calculation_port_list is None:
                ...
                return
            self.logger.info(f"finish get free port")
            self.logger.info(f"Start send config to all parties")
    ...
    def all_mpc_calcalation(self):
        try:
            for i in range(len(self.party_list)):
                ...
                if success is False:
                    return False
                self.logger.info(f"party {self.party_list[i]} join to MPC calculation ")
    ...
    def mpc_calcalation_single_party(self, party_id, i):
        ...
        inter_query_url = "http://{}:{}/1.0/mpc/job/arith".format(party_ip, party_port)
